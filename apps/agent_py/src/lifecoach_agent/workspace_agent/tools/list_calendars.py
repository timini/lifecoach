...
This is intentionally separate from ``list_events``: requests such as
"show my calendar IDs" need ``calendar.calendarList.list`` rather than a
Gmail/search fallback or an events query.
"""
...
def create_list_calendars_tool(deps: WorkspaceToolDeps) -> Any:
    async def list_calendars() -> dict[str, Any]:
        """List Google Calendars available to the user. Returns projected
        calendar-list entries (id, summary, primary, accessRole, timeZone,
        optional description). Read-only.
        """
