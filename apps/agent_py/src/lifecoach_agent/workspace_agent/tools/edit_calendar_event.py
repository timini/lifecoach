...
def create_edit_calendar_event_tool(deps: WorkspaceToolDeps) -> Any:
    async def edit_calendar_event(
        ...
        """
        # Calendar patch updates only the fields we send. Patching `start`
        # alone leaves the old `end` in place, which can produce a zero or
        # negative-duration event — so require `end` whenever `start` changes
        # (Codex P2 on the original PRs).
        if start is not None and end is None:
            return {
                "status": "error",
                "code": "invalid_args",
                "message": "Provide both start and end when changing the event time.",
            }
        ...
        result = await run_gws(
            ...
            resource="events",
            method="patch",
            # sendUpdates=all so existing guests are notified of a time /
            # detail change (and newly-added guests get their invite).
...
def _merge_attendees(existing: Any, add_emails: list[str]) -> list[dict[str, Any]]:
    """Merge new guest emails into the existing attendee list.

    Existing attendees are carried over verbatim (full dict) so per-guest
    metadata — responseStatus, optional, organizer, displayName — survives
    the patch. Calendar's events.patch replaces the whole `attendees` array,
    so rebuilding it from bare emails would silently reset everyone's RSVP
    (Codex P2 on the original PRs). New guests are appended as `{email}`.
    """
