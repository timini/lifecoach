"""Eval harness for the `find_workspace` workspace sub-agent.

This targets calendar-list routing specifically: a calendar ID request should
use `list_calendars`, not Gmail search or event listing.
"""
...
def _stub_before_tool(
    ...
    if name in {"search_messages", "list_events", "list_tasks", "get_message", "list_inbox"}:
        return {"status": "ok", "messages": [], "events": [], "tasks": []}
    return None
...
def _build_eval_root_agent() -> Any:
    deps = WorkspaceToolDeps(store=_FakeTokensStore(), uid="eval-find-workspace-uid")  # type: ignore[arg-type]
    agent = create_workspace_agent(
        deps=deps,
        name=FIND_WORKSPACE_TOOL_NAME,
        description="Workspace finder sub-agent (eval).",
