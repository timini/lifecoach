...
EXCEPTION_LINE_RE = re.compile(r"^(?P<type>[\w.]+(?:Error|Exception|Exit|Interrupt)?): (?P<value>.*)$")
...
def build_sentry_event(entry: dict[str, Any]) -> dict[str, Any]:
    ...
    event: dict[str, Any] = {
        ...
        "fingerprint": [error_group_id] if error_group_id else [labels["service"] or "cloud_run", text[:120]],
        "tags": {
            "source": "cloud_logs",
