...
2. Build a seen_message_ids set from the list_inbox results. For each distinct id, call get_message at most once (e.g. id=<the id from step 1>) to read the decoded body and headers. Parallel calls are fine, but never re-read duplicate ids.
