...
| `POST /background/scheduler/tick` | **Google OIDC** | — | `{status, dispatched}` | ADR 0001. Cloud Scheduler → the dispatcher (`background/dispatcher.py`): sweep due schedules, claim a lease, create the run, enqueue a Cloud Task, advance `nextRunAt`. Bypasses the `x-agent-internal-bearer` gate; verifies OIDC (`aud`==agent URL). `dispatched` is the count enqueued (0 when no schedules are due / no dispatcher configured). |
| `POST /background/runs/{runId}/execute` | **Google OIDC** | `{scheduleId, uid, kind, scheduledFor}` | `{status, runId, errorCode?}` | ADR 0001. Cloud Tasks → the runner (`background/runner.py`): claim lease → validate (enabled / Workspace connected / token usable / workflow registered) → run workflow → persist digest + proposed actions → mark terminal. `skipped`/`succeeded`/`terminal_failed` → 200 (no retry); `retryable_failed` → 5xx (Cloud Tasks retries). Same OIDC gate. |
...
> **Background executor** (`background/runner.py`, `CreateAppDeps.background_runner`). Runs one idempotent run: `claim_for_execution` (a duplicate/in-flight delivery → no-op 200) → validate **before any external I/O** (schedule `enabled`, Workspace connected, token usable, workflow registered; any failure → `skipped` + 200 so Cloud Tasks doesn't retry a non-retryable condition) → run the workflow → persist the digest + proposed actions → `mark_succeeded`. A workflow signals a transient failure by raising `BackgroundRetryableError` → `retryable_failed` + 5xx; a deterministic workflow exception → `terminal_failed` + 200 (the sanitized error class goes to Cloud Logging, never the raw message to Firestore). Everything after a successful claim runs under one guard: once the lease is live, *any* unhandled storage/read failure (a `schedules.get`, a `tokens.get`, a `mark_*`) is converted to `retryable_failed` (which clears the lease) + 5xx rather than escaping as a bare 500 — a 500 would let the re-delivery hit the live lease, get a `noop` 200 ack, and strand the run. On every terminal outcome (`succeeded`/`skipped`/`terminal_failed`) the executor also mirrors the result onto the schedule (`lastStatus`/`lastRunAt`, best-effort — the run record is the source of truth). The app owns every terminal state — Cloud Tasks is never a dead-letter. Workflows implement `BackgroundWorkflow` (`background/workflow.py`) and receive a server-built `BackgroundRunContext` (schedule, run, a valid Workspace access token that never leaves the process). `email_triage_daily` is registered in production: it checks `backgroundConfig/global.allowlistUids`, reads at most 15 projected inbox messages in the configured lookback window, classifies them with `gemini-3.6-flash-lite-preview`, and persists only short digest projections plus proposed actions allowed by the schedule; it never performs a Workspace write. When no runner is wired the execute endpoint returns **503** (not a 200 ack) so a task in flight when config changes isn't silently dropped.

> **Background dispatcher** (`background/dispatcher.py`, `CreateAppDeps.background_dispatcher`). The pure coordinator behind the tick: `query_due` (bounded `.limit().order_by(nextRunAt)`) → `claim_lease` (required dedupe) → create the `backgroundRuns` record → enqueue one Cloud Task (`background/tasks_client.py`; `ALREADY_EXISTS`→success) → `release_lease_and_advance` (scheduling only — the executor owns the run *outcome*). All ids are deterministic per occurrence (`background/ids.py`): the run id, idempotency key, and task id are pure functions of `(scheduleId, kind, scheduledFor)`, so a crash mid-dispatch re-derives the same ids and re-runs idempotently. `nextRunAt` is rolled by `background/schedule_time.py` (DST-aware, in the schedule's IANA zone). The prod Firestore binding is `storage/background_firestore_adapter.py` (over `firestore.AsyncClient`); `main.py` builds the dispatcher only when the full background env set is present (audience, invoker SA, queue, location, project) **and** Workspace OAuth is configured — symmetric with the runner, so the tick never enqueues runs the executor can't run; otherwise the tick is a no-op.
...
| **`subagent_triage_basic_classification`** | Targets the **triage sub-agent directly** (not via the AgentTool). Mocks `list_inbox` + `get_messages` via `before_tool_callback` with a 4-message inbox; asserts the sub-agent lists once, bulk-fetches with one `get_messages`, and emits `<TRIAGE_REPORT>` in its final response. See `tests/evals/eval_triage_inbox_agent.py` for the stubs. |
