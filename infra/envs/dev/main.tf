...
# --- Background work: Cloud Scheduler tick (ADR 0001, step 4b) ------------
# Dev cadence is 15 min (prod pins 5 min). retry_count=0 + 60s deadline; the
# dispatcher's lease + Cloud Tasks dedupe make Scheduler retries unsafe.
module "background_scheduler" {
  source             = "../../modules/background-scheduler"
  project_id         = var.project_id
  region             = var.region
  schedule           = var.background_scheduler_cron
  agent_url          = local.background_oidc_audience
  scheduler_sa_email = module.background_iam.scheduler_sa_email

  depends_on = [module.apis, module.background_iam]
}

# --- Background work: Cloud Tasks run queue (ADR 0001, step 4c) -----------
# Per-run execution queue. The dispatcher enqueues one task per due run; each
# is delivered to /background/runs/{id}/execute with a per-task OIDC token.
# Rate/concurrency caps keep background load off the foreground chat path.
module "background_tasks_queue" {
