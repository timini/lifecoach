...
# --- Cloud Logs -> Sentry -------------------------------------------------
# Forwards Cloud Run ERROR+ log entries that the in-process SDK can miss
# (stderr-only tracebacks, OOM/crashes, and Google infra errors) to Sentry.
# Disabled automatically when sentry_dsn is empty.
