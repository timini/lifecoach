...
- `workspace_connected` → use the eight workspace tools directly; no auth handshake needed.
...
| `edit_calendar_event` | `{eventId, summary?, start?, end?, location?, description?, attendees?, calendarId?, sendUpdates?}` | `calendar.events.patch`. Patches only supplied fields; use after confirming the exact edit. | no |
| `delete_calendar_event` | `{eventId, calendarId?, sendUpdates?}` | `calendar.events.delete`. Use only after explicit confirmation. | no |
...
```
workspace_agent/
  __init__.py              create_workspace_tools(deps) → 8 tools  +  WORKSPACE_TOOL_NAMES
...
|---|---|---|---|---|
| `day_planning` | profile flag `practices.day_planning.enabled` | 05:00–10:59 local | `practices.day_planning.last_planned_date` | none — uses main-agent + the eight workspace tools (when `workspace_connected`) |
