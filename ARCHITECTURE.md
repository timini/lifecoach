...
| `triage_inbox` | `{since?: "1d" \| "12h" \| ...}` | AgentTool wrapping a workspace sub-agent. Returns a structured `TriageReport` with noise / actions / events / info buckets (read-only). Every row carries `from`, `subject`, and a required one-line `context` plus optional `receivedAt` / `snippet` so archive confirmations can list exactly what would be archived. | no |
...
- **`triage_inbox()`** and **`find_workspace(query)`** are `AgentTool`s. Each wraps a dedicated `LlmAgent` instance with a scoped instruction and the same five internal read tools — the sub-agent classifies an inbox or answers a natural-language lookup, the wrapper returns the result to the parent. `triage_inbox` emits `<TRIAGE_REPORT>{json}</TRIAGE_REPORT>`; `parse_triage_report` validates against `TriageReportSchema` and returns `{status:"ok", report}` or `{status:"parse_error", raw}`. The schema requires each triage row to include sender, subject, and a one-line user-facing `context` (optionally backed by `receivedAt` / `snippet`) so the parent can ask `ask_single_choice_question` with an informed per-message archive list rather than a context-free count.
