...
#
# Complements the in-process Sentry SDKs by catching stderr-only library
# tracebacks, container OOM/SIGKILL exits before SDK flush, and Google
# infra-side errors that only appear in Cloud Logging. Disabled at the
# caller (count=0) when var.sentry_dsn is empty.
...
# Pub/Sub push subscriptions authenticated to a Cloud Functions (2nd gen)
# HTTP URI require BOTH:
#   - roles/cloudfunctions.invoker on the function itself (per Google's
#     Pub/Sub push docs, which distinguish Cloud Run from Cloud Run
#     functions), and
#   - roles/run.invoker on the underlying Cloud Run service that
#     Cloud Functions (2nd gen) provisions.
# Without the cloudfunctions.invoker grant, authenticated push requests
# are rejected at the Cloud Functions layer and the forwarder never
# receives messages. (PR 112 review P1.)
resource "google_cloudfunctions2_function_iam_member" "push_cloudfunctions_invoker" {
  project        = var.project_id
  location       = var.region
...
resource "google_cloud_run_v2_service_iam_member" "push_run_invoker" {
  project  = var.project_id
  location = var.region
...
resource "google_pubsub_subscription" "push_to_function" {
  ...
  depends_on = [
    google_cloudfunctions2_function_iam_member.push_cloudfunctions_invoker,
    google_cloud_run_v2_service_iam_member.push_run_invoker,
